...
class CrossModalFusion(nn.Module):
    ...
    def forward(self, t, a, v, mask, valid, return_embedding: bool = False):
        ...
        quality = torch.cat([observed, fractions[:, None].expand(-1, 50, -1)], -1)
        ...
        if return_embedding:
            return outputs + (F.normalize(pooled, dim=-1), self.ordinal(pooled))
        return outputs
...
def class_balanced_order(labels, ids, rng, class_power=0.35, group_power=0.10):
    """Sample an epoch with mild class and video-group balancing.

    Replacement sampling changes only the training order/distribution; valid
    samples and all evaluation scenarios remain untouched.  The small powers
    avoid the high-variance behavior of a fully balanced sampler.
    """
    labels = np.asarray(labels, dtype=np.int64)
    counts = np.bincount(labels, minlength=3).astype(np.float64)
    class_weight = counts[labels] ** (-class_power)
    groups = np.asarray([str(x).split('$_$', 1)[0] for x in ids])
    _, inverse = np.unique(groups, return_inverse=True)
    group_counts = np.bincount(inverse).astype(np.float64)
    group_weight = group_counts[inverse] ** (-group_power)
    probability = class_weight * group_weight
    probability /= probability.sum()
    return rng.choice(len(labels), size=len(labels), replace=True, p=probability)
...
def train(args):
    ...
    config = {
        "experiment": "full_mosei_cross_modal_mild_class_group_balancing",
        ...
        "architecture": f"three modality tokens per word, 2-layer 8-head cross-modal Transformer, 2-layer temporal Transformer, hidden={args.hidden}, dropout={args.dropout}",
        "loss": f"frequency weighted focal CE gamma={args.focal_gamma:g}, power={args.weight_power:g}, label_smoothing={args.label_smoothing:g}, 0.6 SmoothL1, ordinal={args.ordinal_weight:g}, supervised_contrastive={args.supcon_weight:g}, distillation_weight={args.distill_weight:g}",
        "optimizer": f"AdamW lr={args.lr:g}, weight_decay=1e-3, batch={args.batch_size}, clip=1, patience={args.patience}",
        "augmentation": f"{args.clean_prob:g} clean, {args.contiguous_prob:g} contiguous modality subsets, {1-args.clean_prob-args.contiguous_prob:g} scattered shared TAV gaps",
        "sampling": f"class inverse-frequency power={args.sample_class_power:g}; video inverse-group power={args.sample_group_power:g}",
    ...
    (out / "问题2_跨模态交互实验配置.json").write_text(
        json.dumps(config, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    model = CrossModalFusion(hidden=args.hidden, dropout=args.dropout).to(device)
    ...
    stale = 0
    logs = []
    best_path = out / "问题2_跨模态交互完整优化_best.pt"
    last_path = out / "问题2_跨模态交互完整优化_last.pt"
    for epoch in range(1, args.epochs + 1):
        model.train()
        order = class_balanced_order(
            train_split["y"], train_split["ids"],
            np.random.default_rng(args.seed + epoch),
            args.sample_class_power, args.sample_group_power,
        )
        losses = []
        for start in range(0, len(order), args.batch_size):
            indices = order[start:start + args.batch_size]
            mask, text, changed = controlled_augment(
                train_split, indices, *banks, args.clean_prob, args.contiguous_prob
            )
            logits, regression, embedding, ordinal = model(
            ...
            focal_factor = (1.0 - torch.exp(-per_sample)).pow(args.focal_gamma)
            loss = (per_sample * focal_factor).mean() + 0.6 * F.smooth_l1_loss(regression, r)
            if args.supcon_weight:
                loss = loss + args.supcon_weight * supervised_contrastive_loss(embedding, y)
            if args.ordinal_weight:
                ordinal_targets = torch.stack([(y >= 1).float(), (y >= 2).float()], dim=1)
                loss = loss + args.ordinal_weight * F.binary_cross_entropy_with_logits(
                    ordinal, ordinal_targets
                )
            ...
            if eligible.any():
                ...
                loss = loss + args.distill_weight * 0.5 * F.smooth_l1_loss(
                    regression[selected],
        ...
        logs.append(row)
        write_csv(out / "问题2_跨模态交互训练记录.csv", logs)
        write_csv(out / "问题2_跨模态交互验证场景.csv",
    ...
    write_csv(out / "问题2_跨模态交互最佳验证场景.csv",
    ...
    (out / "问题2_跨模态交互实验结论.json").write_text(
        json.dumps(final, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (out / "问题2_跨模态交互模型哈希.json").write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-class-power", type=float, default=0.35)
    parser.add_argument("--sample-group-power", type=float, default=0.10)
    parser.add_argument("--focal-gamma", type=float, default=0.0)
    parser.add_argument("--clean-prob", type=float, default=0.35)
    parser.add_argument("--contiguous-prob", type=float, default=0.30)
