...
def main():
    ...
    fig.suptitle('优化模型在不同缺失条件下的分类性能',fontsize=9,fontweight='bold')
    ...
    axes[0].set(xlabel='连续缺失比例（%）',ylabel='宏平均F1',title='（甲）缺失比例与模态组合')
    ...
    axes[1].set(ylabel='宏平均F1',title='（乙）缺失类型（30%）')
    ...
    fig,axes=plt.subplots(1,2,figsize=(7.4,2.9),constrained_layout=True)
    fig.suptitle('优化模型的验证集分类与强度预测',fontsize=9,fontweight='bold')
    ...
    axes[0].set(xlabel='预测极性',ylabel='真实极性',title='（甲）验证集混淆矩阵')
    ...
    axes[1].set(xlim=(-3.1,3.1),ylim=(-3.1,3.1),xlabel='真实强度',ylabel='预测强度',title='（乙）验证集强度回归')
