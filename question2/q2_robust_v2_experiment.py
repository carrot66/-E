...
DEFAULT_OUT = OUTPUT_ROOT / "问题2_跨模态交互完整优化"
DEFAULT_TEACHER = OUTPUT_ROOT / "问题2_完整MOSEI训练实验/完整数据_缺失蒸馏学生_种子2026.pt"


class RobustFusionV2(nn.Module):
    """Quality-gated cross-modal Transformer for missing/noisy modalities.

    The gate receives both global availability and local continuity.  This is
    useful for MOSEI because zero-valued visual rows and speech gaps are
    genuine missing observations rather than ordinary feature values.
    """
    ...
    def __init__(self, hidden: int = 128, dropout: float = 0.2, layers: int = 2):
        ...
        # observed(3), global fractions(3), local continuity(3), position, valid
        self.quality_projection = nn.Sequential(
            nn.Linear(11, hidden), nn.LayerNorm(hidden), nn.GELU()
        )
        self.modality_gates = nn.ModuleList([
            nn.Sequential(nn.Linear(hidden + 4, max(32, hidden // 2)),
                          nn.GELU(), nn.Linear(max(32, hidden // 2), 1))
            for _ in range(3)
        ])
    ...
    def forward(self, t, a, v, mask, valid, return_embedding: bool = False):
        observed = mask.float()
        denominator = valid.sum(1, keepdim=True).clamp_min(1).float()
        fractions = observed.sum(1) / denominator
        prev = F.pad(observed[:, :-1], (0, 0, 1, 0))
        nxt = F.pad(observed[:, 1:], (0, 0, 0, 1))
        continuity = observed * (prev + nxt) / 2.0
        position = torch.linspace(0, 1, 50, device=t.device)[None, :, None]
        quality = torch.cat([
            observed, fractions[:, None].expand(-1, 50, -1), continuity,
            position.expand(t.shape[0], -1, -1), valid.float()[:, :, None]
        ], -1)
        quality_word = self.quality_projection(quality)

        hs = []
        for j, (proj, gate, x) in enumerate(zip(self.projections,
                                                  self.modality_gates,
                                                  (t, a, v))):
            projected = proj(x)
            gate_quality = torch.stack([
                observed[:, :, j], fractions[:, j, None].expand(-1, 50),
                continuity[:, :, j], valid.float()
            ], -1)
            # Residual gate: a valid modality is never reduced below 50% of
            # its projected signal.  This avoids the cold-start collapse seen
            # when a newly initialized sigmoid gate multiplies features by
            # roughly 0.5 throughout the first epochs.
            gate_value = torch.sigmoid(gate(torch.cat([projected, gate_quality], -1)))
            gate_value = 0.5 + 0.5 * gate_value
            hs.append(projected * gate_value * observed[:, :, j, None])
        ...
        position_token = self.word_position.repeat(1, 3, 1)
        quality_token = quality_word[:, None].expand(-1, 3, -1, -1).reshape(
            t.shape[0], 150, self.hidden
        )
        tokens = tokens + modality + position_token + quality_token
        padding = ~token_mask
        ...
        pooled = self.head((word * attention[..., None]).sum(1))
        outputs = (self.cls(pooled), 3 * torch.tanh(self.reg(pooled).squeeze(-1) / 3))
        if return_embedding:
            return outputs + (F.normalize(pooled, dim=-1),)
        return outputs
...
def controlled_augment(train, indices, blocks, scattered, clean_prob, contiguous_prob):
    """Sample a declared clean/contiguous/scattered mixture for ablations."""
    ...
    for row, sample in enumerate(indices):
        ...
        if draw < clean_prob:
            continue
        if draw < clean_prob + contiguous_prob:
...
def balanced_order(ids, rng, power=0.5):
    """Return a replacement order that prevents long videos dominating epochs."""
    groups = np.asarray([str(x).split('$_$', 1)[0] for x in ids])
    _, inverse = np.unique(groups, return_inverse=True)
    counts = np.bincount(inverse).astype(np.float64)
    sample_weight = counts[inverse] ** (-power)
    sample_weight /= sample_weight.sum()
    return rng.choice(len(ids), size=len(ids), replace=True, p=sample_weight)


def robust_regression_loss(pred, target, labels, mask):
    """Downweight unreliable regression targets and heavily missing examples.

    The audit found roughly 20% disagreement between the supplied regression
    value and the supplied class.  Classification is therefore the primary
    objective; regression is auxiliary and receives a quality/consistency
    weight instead of forcing the two labels to agree.
    """
    expected = torch.as_tensor((-1.5, 0.0, 1.5), device=pred.device)[labels]
    agreement = torch.exp(-torch.abs(pred.detach() - expected) / 1.5)
    observed_quality = mask.float().mean(dim=(1, 2))
    sample_weight = (0.35 + 0.65 * observed_quality) * (0.5 + 0.5 * agreement)
    pointwise = F.smooth_l1_loss(pred, target, reduction='none')
    return (pointwise * sample_weight).mean()


def supervised_contrastive_loss(embedding, labels, temperature: float = 0.12):
    ...
    return -((log_prob * positives).sum(1)[valid] / counts[valid]).mean()
...
def train(args):
    ...
    config = {
        "experiment": "full_mosei_robust_v2_residual_quality_gate_mild_group_balance",
        ...
        "architecture": f"quality-gated three modality tokens per word, 2-layer 8-head cross-modal Transformer, 2-layer temporal Transformer, hidden={args.hidden}, dropout={args.dropout}",
        "loss": f"frequency weighted CE power={args.weight_power:g}, label_smoothing={args.label_smoothing:g}, quality-weighted SmoothL1 weight={args.reg_weight:g}, distillation_weight={args.distill_weight:g}",
        "optimizer": f"AdamW lr={args.lr:g}, weight_decay=1e-3, batch={args.batch_size}, clip=1, patience={args.patience}",
        "sampling": f"video-group replacement sampler with inverse group-size power={args.group_power:g}",
        "augmentation": f"curriculum clean {args.clean_prob_start:g}->{args.clean_prob_end:g}, contiguous {args.contiguous_prob_start:g}->{args.contiguous_prob_end:g}, remainder scattered TAV gaps",
    ...
    (out / "问题2_鲁棒V2实验配置.json").write_text(
        json.dumps(config, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    model = RobustFusionV2(hidden=args.hidden, dropout=args.dropout).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-3)
    counts = np.bincount(train_split["y"], minlength=3).astype(np.float64)
    weights = torch.as_tensor(
        (len(train_split["y"]) / (3 * counts)) ** args.weight_power,
        dtype=torch.float32, device=device
    )
    best_score = -float("inf")
    stale = 0
    logs = []
    best_path = out / "问题2_鲁棒V2质量门控_best.pt"
    last_path = out / "问题2_鲁棒V2质量门控_last.pt"
    for epoch in range(1, args.epochs + 1):
        model.train()
        # Long videos contain many correlated clips.  Sampling by inverse
        # group size avoids letting a handful of videos dominate an epoch.
        order = balanced_order(
            train_split["ids"], np.random.default_rng(args.seed + epoch), args.group_power
        )
        progress = 0.0 if args.epochs <= 1 else (epoch - 1) / (args.epochs - 1)
        clean_prob = args.clean_prob_start + progress * (
            args.clean_prob_end - args.clean_prob_start
        )
        contiguous_prob = args.contiguous_prob_start + progress * (
            args.contiguous_prob_end - args.contiguous_prob_start
        )
        losses = []
        for start in range(0, len(order), args.batch_size):
            indices = order[start:start + args.batch_size]
            mask, text, changed = controlled_augment(
                train_split, indices, *banks, clean_prob, contiguous_prob
            )
            logits, regression, embedding = model(
                *tensor_batch(train_split, indices, device, text, mask),
                return_embedding=True,
            )
            y = torch.as_tensor(train_split["y"][indices], device=device)
            r = torch.as_tensor(train_split["r"][indices], device=device)
            ce_loss = F.cross_entropy(
                logits, y, weight=weights, label_smoothing=args.label_smoothing
            )
            reg_loss = robust_regression_loss(regression, r, y,
            ...
            if eligible.any():
                ...
                loss = loss + args.distill_weight * 4 * F.kl_div(
                    F.log_softmax(logits[selected] / 2.0, -1),
                    target_prob, reduction="batchmean"
                )
                loss = loss + args.distill_weight * 0.25 * F.smooth_l1_loss(
                    regression[selected],
        ...
        row = {
            ...
            "contiguous_probability": float(contiguous_prob),
        }
        logs.append(row)
        write_csv(out / "问题2_鲁棒V2训练记录.csv", logs)
        write_csv(out / "问题2_鲁棒V2验证场景.csv",
    ...
    write_csv(out / "问题2_鲁棒V2最佳验证场景.csv",
              [{"stage": "best", **r} for r in old_rows + sparse_rows])
    baseline = float(args.baseline_score)
    final = {
        "selected": "robust_v2" if result["selection_score"] >= baseline + args.min_gain else "previous_full_student",
        "robust_v2_profile": result,
    ...
    (out / "问题2_鲁棒V2实验结论.json").write_text(
        json.dumps(final, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (out / "问题2_鲁棒V2模型哈希.json").write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--group-power", type=float, default=0.5)
    parser.add_argument("--clean-prob-start", type=float, default=0.55)
    parser.add_argument("--clean-prob-end", type=float, default=0.35)
    parser.add_argument("--contiguous-prob-start", type=float, default=0.25)
    parser.add_argument("--contiguous-prob-end", type=float, default=0.30)
    parser.add_argument("--baseline-score", type=float, default=0.5702501057)
