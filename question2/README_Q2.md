...
数据预处理另有 [5 张中文结果图](../outputs/question2/问题2_预处理结果图/)：来源与划分、训练验证类别构成、原始模态不可用比例、五折视频组划分、安全整合流程。运行 `python question2/q2_preprocessing_figures.py` 可用已归档的审计 JSON 重绘；[预处理配图说明](问题2_预处理配图说明.md)给出数据口径。保留测试集和附件3的标签均未读取。
