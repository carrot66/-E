"""Q3 token-interaction entrypoint; target checked against official validation."""
import argparse
import os
import sys
import json
...
def main():
    ...
    p.add_argument('--out',type=Path,default=ROOT/'outputs/question3/q3_final_01')
    p.add_argument('--device',default='cuda:0')
    p.add_argument('--epochs',type=int,default=20)
    p.add_argument('--patience',type=int,default=8)
    p.add_argument('--batch-size',type=int,default=16)
    p.add_argument('--seeds',type=int,nargs='+',default=[2026,2027,2028])
    from q3_model import candidates
    ...
    if args.stage in ('explain','all'):
        from q3_explain import explain
        explain(args)
    if args.stage in ('report','all'):
        from q3_report import run_report
        run_report(args)
    if args.stage in ('audit','all'):
        from q3_audit import audit_outputs
